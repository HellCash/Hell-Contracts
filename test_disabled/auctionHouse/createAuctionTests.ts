...
describe("[Auction House] function createAuction", () => {
    ...
    it('Creating an Auction with a length lower than 2000 blocks should fail', async () => {
        ...
        await expect(auctionHouseContract.createAuction(
            ...
            parseEther("50"), // Starting Bid Price
            BigNumber.from(0), // Buyout price
            await ethers.provider.getBlockNumber() + 1999) // 1999 blocks from now.
        ).to.be.revertedWith('CA3');
    });
    ...
    it('Creating an Auction against the same asset should fail', async () => {
        await expect(auctionHouseContract.createAuction(
            ...
            .to.be.revertedWith('CA4');
    });

    it('ETHER: Creating an Auction without sending enough funds should fail', async () => {
        await expect(auctionHouseContract.createAuction(
    ...
    it('ERC20: Creating an Auction without enough balance should fail', async () => {
    ...
    it('ETHER: Should Auction Ether Successfully',async () => {
        ...
        const auctionedAmount = parseEther("35");
    ...
    it('ERC20: Should Auction a ERC20 token Successfully',async () => {
